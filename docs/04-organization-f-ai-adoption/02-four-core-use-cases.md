...
Cập nhật 2026-09-09. Đọc trước [Direction and Scope](01-direction-and-scope.md). Bốn việc của team dev là lõi của đợt này; role khác ở mục 6 dưới dạng ý tưởng. Mã T, M, K là artifact ở [03, mục 1](03-work-items-and-pic.md#1-artifact). Skill của MK nêu tên để biết điểm xuất phát; mức sẵn sàng đánh giá từ nội dung skill, chưa chạy trên artifact của team nào nên đều là **[chưa kiểm chứng]** cho tới bước khảo sát.
...
| Cần từ team | Ba mẫu thiết kế chi tiết đã được duyệt; danh sách mục bắt buộc; ai duyệt thiết kế (T2, T3) |
| Đo | Số chỉ trích ở review thiết kế trên một tài liệu; giờ làm một tài liệu, trước và sau (T4, M8) |
| Material | Module M4 cho dev lead; prompt template M5; khuôn thiết kế trong khu vực team T5 |
...
| Cần từ team | Ba mẫu đặc tả test đã duyệt; quy tắc đặt tên và độ mịn của test case; ai duyệt (T2, T3) |
| Đo | Tỷ lệ test case do AI sinh được duyệt trên tổng test case của pha; giờ làm (T4, M8) |
| Material | Module M4 cho tester; prompt template M5; skill K4 và khuôn đặc tả test trong T5 |
...
| Cần từ team | Coding standard, review checklist, một module mẫu đã qua review, cách chạy build (T2, T3) |
| Đo | Lỗi review trên nghìn dòng; giờ code một hạng mục, trước và sau (T4, M8) |
| Material | Module M4 cho dev; prompt template M5; rule và ngữ cảnh trong T5 |
...
| Cần từ team | Framework test, cấu trúc thư mục test, ngưỡng coverage, mẫu đặc tả test đơn vị nếu có (T2, T3) |
| Đo | Coverage tại gate test đơn vị; tỷ lệ dự án có unit test tự động; giờ viết test (T4, M8) |
| Material | Module M4 cho dev; prompt template M5; cấu hình test trong T5 |
...
Bảng này thành trang M6 trong bộ material, dạng "AI làm được gì cho role của bạn"; hands-on cho các role này lập ở đợt sau.
